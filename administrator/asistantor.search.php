...
<?php
   ...
   $key = $_GET['key'];
   $asistantor = fetchAll("select * from user where belong_to like 'asistantor' and (name like '$key%' or number like '%$key%')");
?>
...
<table width="99%" border="0" margin="0" padding="0">
  ...
  <tr>
    ...
    <td>
      <a href="asistantor.mod.php?id=<?=$result['id']?>"><input type="submit" value="编辑"/></a>
      <a href="asiatantor.del.php?id=<?=$result['id']?>"><input type="submit" value="删除" onclick="javaScript:myconfirm();return false;"/></a>
    </td>
  </tr>
  <?php $i+=1; } }?>
</table>
