   ...
   if(move_uploaded_file($tmp_name,iconv("UTF-8","gb2312","../uploads/".$filename)))
   {
      ...
      //  echo "<br />";
        ...
        for($row = 1;$row < $highestRow;$row++)
        {
            ...
            $created=$_SESSION['name'];
              $student = array(
                    ...
                    'institute'=>"'$strs[3]'",//学院
					'major'=>"'$strs[4]'",   //专业
					'political_class'=>"'$strs[5]'",//行政班
				    'grade'=>"'$strs[6]'",   //年级
					'birthday'=>"'$strs[7]'",//生日
				    'political_status'=>"'$strs[8]'",//政治面貌
					'national'=>"'$strs[9]'",  //民族
					'phone'=>"'$strs[10]'",    //联系电话    
					'password'=>"'$strs[0]'",  //密码
                    'belong_to'=>"'student'",
       ...
       $query = insert('user',$student);
        ...
        if(!$query)
                 echo "<script>alert('学生信息导入成功');window.location.href='student.php';</script>";
             else echo "<script>alert('学生信息导入失败');window.location.href='student.php';</script>";
 }else echo "上传过程出错";
?>