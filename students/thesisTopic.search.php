<?php
  ...
  $key = $_GET['key'];
  if($key=='可选') $key1=1;
  else if($key=='不可选') $key1=0;
	$thesis = fetchAll("select * from thesis_title where status = '1' and title like '%$key%' or teacher_id like '$key' or status like '$key1'");
?>

<?php include("header.php");?>
<?php
    $thesis_choose = fetchAll("select * from student_choose_thesis_title where student_id = '$id'");
    //print_r($thesis_choose);
    ?>
<p>
  <?php 
                ...
                <td>
                  <?php 
                     if($result['status']==1) {
                        if(empty($thesis_choose)){
                     ?>
                    <input type="submit" name="choose" value="选择" onclick="javascript:myconfirm();return false;"/>
                  <?php }
                        else echo "不可重复选择";
                    }?>
                </td>
